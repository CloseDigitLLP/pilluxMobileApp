...
export const getVehicles = (id) => async (dispatch) => {
    try{
        dispatch({ type: actionTypes.GET_VEHICLES })
        ...
        console.log(error)
        return Promise.reject(dispatch({ type: actionTypes.GET_VEHICLES_FAILED, payload: error?.response?.data?.message || error?.message }))
    }
}