...
export const getAllEvents = () => async (dispatch) => {
    try{
        ...
        console.log(error)
        return Promise.reject(dispatch({ type: actionTypes.GET_EVENTS_FAILED, payload: error?.response?.data?.message || error?.message }))
    }
}
...
export const updateEvent = (data, id) => async (dispatch) => {
    try{
        ...
        console.log(error)
        return Promise.reject(dispatch({ type: actionTypes.UPDATE_EVENT_FAILED, payload: error?.response?.data?.message || error?.message }))
    }
}